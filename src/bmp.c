...
unsigned char *bmp_get(char *filePath, int *picMaxSize, int *width, int *height, int *per)
{
    ...
    {
		printf("bmp_get : %s read Bmp_FileHeader failed\r\n", filePath);
		close(fd);
	    return NULL;
    }
    ...
    {
        printf("bmp_get : %s bmp type err, bfType must be \"BM\"\r\n", filePath);
		close(fd);
	    return NULL;
    }
    //Bmp_Info
    ...
    if((ret = read(fd, data, totalSize + overLineBytesSum)) != (totalSize + overLineBytesSum))
    {
        if(ret <= 0)
        {
		    printf("bmp_get : %s read data failed\r\n", filePath);
		    free(data);
		    close(fd);
	        return NULL;
        }
...
int bmp_create(char *filePath, unsigned char *data, int width, int height, int per)
{
    ...
    if(width < 0)
    {
        printf("bmp_create : %s width < 0 , err !!\r\n", filePath);
        return -1;
    }
    //
    if((fp = open(filePath, O_WRONLY|O_CREAT, 00700)) < 0)
    {
...
//
unsigned char ***bmp_mapInit(unsigned char *pic, int width, int height, int pw)
{
    unsigned char ***map;
    int xC, yC;
    //
    if(pic == NULL || width < 1 || height < 1 || pw < 1)
        return NULL;
    //
    map = (unsigned char ***)calloc(height, sizeof(unsigned char **));
    for(yC = 0; yC < height; yC++)
    {
        map[yC] = (unsigned char **)calloc(width, sizeof(unsigned char *));
...
//
void bmp_mapRelease(unsigned char ***picMap, int width, int height)
{
    int i;
    //
    if(picMap == NULL)
