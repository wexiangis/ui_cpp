                ...
                if(bits[j] & 1 << k) {
                    ...
                    if(bit_labels && (print_flags & PRINT_LABELS)) {
                        ...
                        if(bit_label)
                            ...
                            printf(" %.20s%c%ld", bit_label, down, (long)(20 - strlen(bit_label)));
                        else
...
static void print_event(int type, int code, int value, int print_flags)
{
    ...
    //else
    //    printf("(else)%04x %04x %08x", type, code, value);
}
...
{
#if(TOUCH_X_MIRROR)
    int xp = ts->xSize - ts->xy[0];
    int xpd = ts->xSize - ts->downXy[0];
#else
    int xp = ts->xy[0];
    int xpd = ts->downXy[0];
#endif
...
#if(TOUCH_Y_MIRROR)
    int yp = ts->ySize - ts->xy[1];
    int ypd = ts->ySize - ts->downXy[1];
#else
    int yp = ts->xy[1];
    int ypd = ts->downXy[1];
#endif
    //
    if(ts->screenDir < 1 || ts->screenDir > 3)
    {
        ts->xyReal[0] = xp;
        ts->xyReal[1] = yp;
        ts->downXyReal[0] = xpd;
    ...
    else if(ts->screenDir == 1)
    {
        ts->xyReal[0] = yp;
        ts->xyReal[1] = ts->xSize - xp;
        ts->downXyReal[0] = ypd;
        ts->downXyReal[1] = ts->xSize - xpd;
    }
    else if(ts->screenDir == 2)
    {
        ts->xyReal[0] = ts->xSize - xp;
        ts->xyReal[1] = ts->ySize - yp;
        ts->downXyReal[0] = ts->xSize - xpd;
        ts->downXyReal[1] = ts->ySize - ypd;
    }
    else
    {
        ts->xyReal[0] = ts->ySize - yp;
        ts->xyReal[1] = xp;
        ts->downXyReal[0] = ts->ySize - ypd;
        ts->downXyReal[1] = xpd;
    }
}
//功能: 整理与拖动相关参数
static void touch_xyErrSort(Touch_Struct *ts)
{
    int xAbs, yAbs;
    int xyErrMatchResultTemp, xyErrMatchValueTemp;
    //
    if(ts->xyErrReal[0] < 0)
        xAbs = -ts->xyErrReal[0];
    else
    ...
    //
    if(xAbs > yAbs && 
        (xAbs > ts->xyErrMatchThreshold || 
        ts->xyErrMatchValueMax > ts->xyErrMatchThreshold ))
    {
        if(ts->xyErrReal[0] > 0)
            xyErrMatchResultTemp = 2;
        else
            xyErrMatchResultTemp = 4;
        xyErrMatchValueTemp = xAbs;
    }
    else if(yAbs > ts->xyErrMatchThreshold && 
        (yAbs > ts->xyErrMatchThreshold || 
        ts->xyErrMatchValueMax > ts->xyErrMatchThreshold ))
    {
        if(ts->xyErrReal[1] > 0)
            xyErrMatchResultTemp = 3;
        else
            xyErrMatchResultTemp = 1;
        xyErrMatchValueTemp = yAbs;
    }
    else
    {
        xyErrMatchResultTemp = 0;
        xyErrMatchValueTemp = 0;
    }
    //
    if(xyErrMatchResultTemp == 0 || xyErrMatchResultTemp != ts->xyErrMatchResult)
        ts->xyErrMatchDivideValue = xyErrMatchValueTemp;
    else
        ts->xyErrMatchDivideValue = xyErrMatchValueTemp - ts->xyErrMatchValue;
    //
    ts->xyErrMatchResult = xyErrMatchResultTemp;
    ts->xyErrMatchValue = xyErrMatchValueTemp;
    //
    if(ts->xyErrMatchValue > ts->xyErrMatchValueMax)
    {
        ts->xyErrMatchValueMax = ts->xyErrMatchValue;
        ts->xyErrMatchResultMax = ts->xyErrMatchResult;
    }
}
//功能: 整理事件, 获得触屏数据
static void touch_eventSort(Touch_Struct *ts, struct input_event *event)
{
    //int xErr, yErr;
    //
    if(ts == NULL || event == NULL)
        return;
    //
    if(event->type == EV_KEY)
    {
        ...
        {
            if(event->value == 0)    // UP 事件
            {
                //printf("[%8ld.%06ld] UUUUUUPPPPPP\r\n", event->time.tv_sec, event->time.tv_usec);
                //
                ts->currentTime[0] = event->time.tv_sec;
                ts->currentTime[1] = event->time.tv_usec;
                //
                if(ts->currentTime[0] > ts->time[0])
                {
                    if(ts->currentTime[1] < ts->time[1])
                        ts->timeErrMs = (1000000 - ts->time[1] + ts->currentTime[1])/1000 + (ts->currentTime[0] - ts->time[0] - 1)*1000;
                    else
                        ts->timeErrMs = (ts->currentTime[1] - ts->time[1])/1000 + (ts->currentTime[0] - ts->time[0])*1000;
                }
                else if(ts->currentTime[1] > ts->time[1])
                    ts->timeErrMs = (ts->currentTime[1] - ts->time[1])/1000;
                else
                    ts->timeErrMs = 0;
                //
                ts->onTouch = false;
            ...
            else if(event->value == 1)   // DOWN 事件
            {
                ...
                ts->xyErrReal[1] = 0;
                //
                ts->time[0] = ts->currentTime[0] = event->time.tv_sec;
                ts->time[1] = ts->currentTime[1] = event->time.tv_usec;
                //
                ts->timeErrMs = 0;
                //
                ts->onTouch = true;
                ...
                ts->touchEventCount = 0;
                //
                ts->xyErrMatchValueMax = 0;
                ts->xyErrMatchResultMax = 0;
                //----------- 触发事件 ----------
                if(ts->downCallBack)
                {
                    // touch_xyErrSort(ts);
    ...
    {
        if(event->code == ABS_MT_POSITION_X)     // X 轴数值输出, 没有输出时沿用之前的坐标
        {
            //printf("[%8ld.%06ld] X = %d\r\n", event->time.tv_sec, event->time.tv_usec, event->value);
            //
            ts->currentTime[0] = event->time.tv_sec;
            ts->currentTime[1] = event->time.tv_usec;
            //
            if(ts->currentTime[0] > ts->time[0])
            {
                ...
                ts->timeErrMs = 0;
            //
            ts->touchEventCount += 1;
            //
            // 当本次点击的xy(其中一轴)坐标与上一次一样时, 本次是不会再输出该轴坐标的
            // 所以可以通过 "touchEventCount" 计数和 "是否收到过该轴坐标" 来决定是否要沿用上一次的坐标值
            if(ts->touchEventCount == 1)
            {
                ts->xy[0] = event->value;
                //
                ts->downXy[0] = ts->xy[0];
            ...
            else if(ts->touchEventCount == 2 && ts->downXy[0] < 0)
            {
                ts->xy[0] = event->value;
                //
                ts->downXy[0] = ts->xy[0];
            ...
            {
                // touchEventCount 已过2(前面两次的坐标都是同一轴上的), 还未收到坐标的沿用上一次的坐标
                if(ts->downXy[0] < 0)
                    ts->downXy[0] = ts->xy[0];
                if(ts->downXy[1] < 0)
                    ts->downXy[1] = ts->xy[1];
                ...
                touch_dirSort(ts);
                //
                ts->xyErr[0] = ts->xy[0] - ts->downXy[0];
                ts->xyErr[1] = ts->xy[1] - ts->downXy[1];
                ts->xyErrReal[0] = ts->xyReal[0] - ts->downXyReal[0];
                ts->xyErrReal[1] = ts->xyReal[1] - ts->downXyReal[1];
                //----------- 触发事件 ----------
                if(ts->movCallBack)
                {
                    touch_xyErrSort(ts);
        ...
        else if(event->code == ABS_MT_POSITION_Y)     // Y轴数值输出, 没有输出时沿用之前的坐标
        {
            //printf("[%8ld.%06ld] Y = %d\r\n", event->time.tv_sec, event->time.tv_usec, event->value);
            //
            ts->currentTime[0] = event->time.tv_sec;
            ...
            else if(ts->touchEventCount == 2 && ts->downXy[1] < 0)
            {
                ts->xy[1] = event->value;
                //
                ts->downXy[1] = ts->xy[1];      //
        ...
        else if(event->code == ABS_MT_TRACKING_ID)     // ID
        {
            printf("[%8ld.%06ld] ID = %d\r\n", event->time.tv_sec, event->time.tv_usec, event->value);
            ts->id = event->value;
        }
        else if(event->code == ABS_MT_TOUCH_MAJOR)     // 接触面长轴
        {
            printf("[%8ld.%06ld] MAJOR = %d\r\n", event->time.tv_sec, event->time.tv_usec, event->value);
        }
        else if(event->code == ABS_MT_TOUCH_MINOR)     // 接触面短轴
        {
            printf("[%8ld.%06ld] MINOR = %d\r\n", event->time.tv_sec, event->time.tv_usec, event->value);
        }*/
    }/*
    else if(event->type == EV_SYN)
    {
        if(event->code == SYN_REPORT)     // 准许上报本次事件
        {
            printf("[%8ld.%06ld] report\r\n", event->time.tv_sec, event->time.tv_usec);
        }
    }*/
}
//线程, 使用poll检测硬件的report事件
static int touch_main(int argc, char *argv[], Touch_Struct *ts)
{
    int c;
    int i;
    int res;
    ...
    printf("=== touch_main : readly === \r\n");

    while(1) 
    {
        //使用poll来阻塞检测input设备事件
        ...
        for(i = 1; i < nfds; i++) 
        {
            if(ufds[i].revents) 
            {
                if(ufds[i].revents & POLLIN) 
                {
                    ...
                    //有限次检测     //这里没有用到, event_count = 0
                    //if(event_count && --event_count == 0)
                    //    return 0;
                    
                    //============= 筛选设备 =============
                    if(1)   //(strstr(device_names[i], "event0"))   //
                    {
                        touch_eventSort(ts, &event);
                    }
                }
            }
        }
...
//创建检测线程函数
void touch_main_thread_func(void *argv)
{
    int ret;
    Touch_Struct *ts = (Touch_Struct *)argv;
    
    //ret = getevent_main(argc, argv);
    ret = touch_main(1, NULL, ts);
    printf("getevent_main : ret/%d\r\n", ret);
    
    return ;
}
//
Touch_Struct *touch_struct = NULL;      //保留自己用
//功能: 总的初始化方法, 之后会自动根据事件类型去调用对应的回调函数
//参数: screenDir: 屏幕倒向, 0 正, 1 右倒, 2 正倒, 3 
...
{
    Touch_Struct *ts = NULL;
    //
    if(touch_struct)
        return 0;
    //
    ts = (Touch_Struct *)calloc(1, sizeof(Touch_Struct));
    //
    ts->xSize = xSize;
    ts->ySize = ySize;
    //
    ts->screenDir = screenDir;
    ts->privateData = privateData;
    //
    ts->upCallBack = upCallBack;
    ts->downCallBack = downCallBack;
    ts->movCallBack = movCallBack;
    //
    if (pthread_create(&ts->th, NULL, (void *)touch_main_thread_func, (void *)ts) != 0)
    {
        printf("\r\ntouch_check_func thread create failed\r\n");
        free(ts);
        return -1;
    }
    //
    ts->xyErrMatchThreshold = 20;
    //
    touch_struct = ts;
    //
    return 0;
